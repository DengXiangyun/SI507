...
% Student Name: Deng, Xiangyun

% Student Unique Name: dengxy
...
def make_move(player, board):
    ...
    a= input(f"{player_name(player)}'s move:")
...
def check_win_horizontal(board):
    ...
    '''check if there are three same marks in a row, 
    if so, return the mark
    if not, return 0

    first check the first row
    then check the second row
    then the third row
...
def check_win_vertical(board):
    # TODO: write docstring
    '''check if there are three same marks in a line, 
    if so, return the mark
    if not, return 0

    first check the first line
    then check the second line
    then the third line
...
def check_win_diagonal(board):
    # TODO: write docstring
    '''check if there are three same marks in a diagonal, 
    if so, return the mark
    if not, return 0

    first check the diagonal from upper left to lower right
...
while(moves_left > 0 and winner == 0):
    ...
    if winner == 0:
        ...
        print(f"{PLAYER_NAMES[winner]} is the winner")
        break
